...
def segment_textract_json_to_dir(raw_path, segmented_dir):
    ...
    - raw_path: str or Path to *_raw.json
    - segmented_dir: directory where segmented file should be written
...
def run_segmentation_all(**context):
    ...
    if not RAW_DIR.exists():
        msg = f"RAW_DIR not found: {RAW_DIR}"
        logger.warning(msg)
    ...
    if not raw_files:
        msg = f"No raw JSON files found in {RAW_DIR}"
        logger.warning(msg)
        track_task(task_name, "FAILED", error=msg)
        return []
